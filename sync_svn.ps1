...
# add default values
param( [string]$local_src_path = "", [string]$remote_path="", [string]$report_file="" )

Function CopyRequired ( $path )
{
    $accepted_extensions = @(".cxx", ".hxx", ".c", ".cpp", ".h", ".hpp", "Makefile", ".sh", ".py", ".txt", ".thor")
    ...
    if ( test-path $path -PathType Leaf) {
        ...
        return (($path -ne "ThorServers") -and (!$path.Contains("nbproject") )  -and (!$path.EndsWith("build\config") )  -and (!$path.EndsWith("build\.metadata") ) -and (!$path.EndsWith("build\var") ) )
    }
...
if ($haveWork)
{
    ...
    write "Copy: $local_file_path"
        if ( test-path $local_file_path -PathType Container) {
            if ( test-path $remote_file_path) {
                Remove-Item -recurse -force $remote_file_path
            }

            Copy-item -recurse -Force $local_file_path $remote_file_path
        } else {
    ...
    foreach ($file in $delete_files) {
        ...
        if (test-path $remote_file_path) {
            if ( test-path $remote_file_path -PathType Container ) {
                Remove-Item -recurse -force $remote_file_path
            } else {
